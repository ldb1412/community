...
//     @Bean
    public JobDetailFactoryBean alphaJobDetail() {
        ...
        factoryBean.setJobClass(AlphaJob.class);//声明管理的任务的类型
        factoryBean.setName("alphaJob");//声明的任务的名字
        factoryBean.setGroup("alphaJobGroup");//给任务取一个组名
        factoryBean.setDurability(true);//声明任务长久保存否
...
//     @Bean
    public SimpleTriggerFactoryBean alphaTrigger(JobDetail alphaJobDetail) {
        ...
        factoryBean.setJobDetail(alphaJobDetail);//对哪个任务进行管理（传入的detail最好和之前配置的detail名字一致）
        factoryBean.setName("alphaTrigger");//对trigger取名字
        factoryBean.setGroup("alphaTriggerGroup");//取组名
        factoryBean.setRepeatInterval(3000);//多久执行一次（3000ms）
    ...
    public SimpleTriggerFactoryBean postScoreRefreshTrigger(JobDetail postScoreRefreshJobDetail) {
        ...
        factoryBean.setRepeatInterval(1000 * 60 * 5);//定时时间间隔
